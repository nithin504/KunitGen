...
#define WAKE_INT_STATUS_REG0 0x100
#define WAKE_INT_STATUS_REG1 0x104
#define WAKE_INT_MASTER_REG  0x108
#define EOI_MASK             0x1
#define PIN_IRQ_PENDING      0x1
#define INTERRUPT_MASK_OFF   16
#define WAKE_STS_OFF         17
...
static int generic_handle_domain_irq_safe_last_irq = 0;
int generic_handle_domain_irq_safe(struct irq_domain *domain, unsigned int hwirq)
{
	generic_handle_domain_irq_safe_call_count++;
	generic_handle_domain_irq_safe_last_irq = hwirq;
	return 0;
}

#include "pinctrl-amd.c"

static char mock_mmio_region[8192];
...
static void setup_mock_gpio_device(void)
{
	...
	generic_handle_domain_irq_safe_last_irq = 0;
...
{
	setup_mock_gpio_device();
	u32 *status_reg0 = (u32 *)(mock_mmio_region + WAKE_INT_STATUS_REG0);
...
static void test_do_amd_gpio_irq_handler_irq_pending_not_masked(struct kunit *test)
{
	...
	*pin_reg = PIN_IRQ_PENDING;
	...
	KUNIT_EXPECT_EQ(test, generic_handle_domain_irq_safe_last_irq, 0);
}
...
static void test_do_amd_gpio_irq_handler_no_irq_pending(struct kunit *test)
{
	setup_mock_gpio_device();
	u32 *status_reg0 = (u32 *)(mock_mmio_region + WAKE_INT_STATUS_REG0);
	u32 *pin_reg = (u32 *)(mock_mmio_region + 0x0);
...
static void test_do_amd_gpio_irq_handler_spurious_irq_disables_interrupt(struct kunit *test)
{
	setup_mock_gpio_device();
	...
	*pin_reg = PIN_IRQ_PENDING | BIT(INTERRUPT_MASK_OFF);
	...
	KUNIT_EXPECT_FALSE(test, result);
	KUNIT_EXPECT_EQ(test, generic_handle_domain_irq_safe_call_count, 1);
	u32 regval = readl(pin_reg);
	KUNIT_EXPECT_EQ(test, (int)(regval & BIT(INTERRUPT_MASK_OFF)), 0);
}

static void test_do_amd_gpio_irq_handler_multiple_pins_one_active(struct kunit *test)
{
	setup_mock_gpio_device();
	u32 *status_reg0 = (u32 *)(mock_mmio_region + WAKE_INT_STATUS_REG0);
...
static void test_do_amd_gpio_irq_handler_multiple_status_bits(struct kunit *test)
{
	...
	u32 *status_reg0 = (u32 *)(mock_mmio_region + WAKE_INT_STATUS_REG0);
	u32 *pin_reg0 = (u32 *)(mock_mmio_region + 0x0);
	u32 *pin_reg4 = (u32 *)(mock_mmio_region + 0x10);
	*status_reg0 = 0x3;
	*pin_reg0 = PIN_IRQ_PENDING | BIT(INTERRUPT_MASK_OFF);
	*pin_reg4 = PIN_IRQ_PENDING | BIT(INTERRUPT_MASK_OFF);
	bool result = do_amd_gpio_irq_handler(10, &mock_gpio_dev);
	KUNIT_EXPECT_TRUE(test, result);
	KUNIT_EXPECT_EQ(test, generic_handle_domain_irq_safe_call_count, 2);
	KUNIT_EXPECT_EQ(test, generic_handle_domain_irq_safe_last_irq, 4);
}

static void test_do_amd_gpio_irq_handler_eoi_written_on_positive_irq(struct kunit *test)
{
	setup_mock_gpio_device();
	u32 *status_reg0 = (u32 *)(mock_mmio_region + WAKE_INT_STATUS_REG0);
	...
	*master_reg = 0x0;
	...
	u32 regval = readl(master_reg);
	KUNIT_EXPECT_EQ(test, (int)(regval & EOI_MASK), 1);
}

static void test_do_amd_gpio_irq_handler_eoi_not_written_on_negative_irq(struct kunit *test)
{
	setup_mock_gpio_device();
	u32 *status_reg0 = (u32 *)(mock_mmio_region + WAKE_INT_STATUS_REG0);
	u32 *master_reg = (u32 *)(mock_mmio_region + WAKE_INT_MASTER_REG);
	...
	*master_reg = 0x0;
	...
	u32 regval = readl(master_reg);
	KUNIT_EXPECT_EQ(test, (int)(regval & EOI_MASK), 0);
}

static struct kunit_case do_amd_gpio_irq_handler_test_cases[] = {
	...
	KUNIT_CASE(test_do_amd_gpio_irq_handler_no_irq_pending),
	KUNIT_CASE(test_do_amd_gpio_irq_handler_spurious_irq_disables_interrupt),
	KUNIT_CASE(test_do_amd_gpio_irq_handler_multiple_pins_one_active),
	KUNIT_CASE(test_do_amd_gpio_irq_handler_multiple_status_bits),
	KUNIT_CASE(test_do_amd_gpio_irq_handler_eoi_written_on_positive_irq),
	KUNIT_CASE(test_do_amd_gpio_irq_handler_eoi_not_written_on_negative_irq),
	{}
};

static struct kunit_suite do_amd_gpio_irq_handler_test_suite = {
	.name = "do_amd_gpio_irq_handler_test",
	.test_cases = do_amd_gpio_irq_handler_test_cases,
};

kunit_test_suite(do_amd_gpio_irq_handler_test_suite);
