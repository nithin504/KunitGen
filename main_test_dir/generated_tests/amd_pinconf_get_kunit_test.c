...
static void test_amd_pinconf_get_input_debounce(struct kunit *test)
{
	...
	unsigned long packed = pinconf_to_config_packed(PIN_CONFIG_INPUT_DEBOUNCE, reg_val & DB_TMR_OUT_MASK);
	KUNIT_EXPECT_EQ(test, config, packed);
}
...
static void test_amd_pinconf_get_bias_pull_down_enabled(struct kunit *test)
{
	...
	unsigned long packed = pinconf_to_config_packed(PIN_CONFIG_BIAS_PULL_DOWN, 1);
	KUNIT_EXPECT_EQ(test, config, packed);
}
...
static void test_amd_pinconf_get_bias_pull_down_disabled(struct kunit *test)
{
	...
	unsigned long packed = pinconf_to_config_packed(PIN_CONFIG_BIAS_PULL_DOWN, 0);
	KUNIT_EXPECT_EQ(test, config, packed);
}
...
static void test_amd_pinconf_get_bias_pull_up_enabled(struct kunit *test)
{
	...
	unsigned long packed = pinconf_to_config_packed(PIN_CONFIG_BIAS_PULL_UP, 1);
	KUNIT_EXPECT_EQ(test, config, packed);
}
...
static void test_amd_pinconf_get_bias_pull_up_disabled(struct kunit *test)
{
	...
	unsigned long packed = pinconf_to_config_packed(PIN_CONFIG_BIAS_PULL_UP, 0);
	KUNIT_EXPECT_EQ(test, config, packed);
}
...
static void test_amd_pinconf_get_drive_strength(struct kunit *test)
{
	...
	u32 reg_val = (0x5 << DRV_STRENGTH_SEL_OFF);
	...
	unsigned long packed = pinconf_to_config_packed(PIN_CONFIG_DRIVE_STRENGTH, 0x5);
	KUNIT_EXPECT_EQ(test, config, packed);
}
